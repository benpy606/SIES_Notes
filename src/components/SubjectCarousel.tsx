...
export default function SubjectCarousel({
  ...
  return (
    <div className="bg-slate-950 border-b border-slate-800 sticky top-[61px] z-20 transition-colors px-4 py-3">
      <div className="flex gap-2.5 overflow-x-auto no-scrollbar py-0.5 items-center">
        <button
          ...
          className={`min-h-[40px] px-4 rounded-xl text-xs font-black transition-all shrink-0 flex items-center justify-center hover-bounce font-display ${
            selected === 'All'
              ...
              : 'bg-slate-900 text-slate-100 border border-slate-800 hover:border-slate-700 hover:bg-slate-850 shadow-xs'
          }`}
        >
          ✨ All Subjects
        </button>
        {subjects.map((sub) => {
          ...
          return (
            ...
            >
              <span
                className="w-3 h-3 rounded-full ring-2 ring-white/60 dark:ring-white/30 shadow-xs transition-transform"
