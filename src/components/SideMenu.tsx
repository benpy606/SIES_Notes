...
import { X, LogOut, BookOpen, Upload, FileText, Image as ImageIcon, Shield, User, Filter, Sparkles } from 'lucide-react'
...
export default function SideMenu({
  ...
  return (
    <div className="fixed inset-0 z-50 flex animate-fade-in">
      ...
      {/* Drawer Content */}
      <div className="relative w-84 max-w-[88vw] bg-slate-950 border-r border-slate-800 text-slate-100 h-full flex flex-col shadow-2xl z-10 animate-slide-up overflow-y-auto">
        {/* Drawer Header */}
        <div className="p-5 border-b border-slate-800/90 flex items-center justify-between bg-slate-900/90 backdrop-blur-md sticky top-0 z-10">
          ...
          <div className="space-y-1.5">
            ...
            >
              <div className="flex items-center gap-2.5">
                <Sparkles size={14} className={selectedSubject === 'All' ? 'text-amber-400' : 'text-slate-500'} />
            ...
            {subjects.map((sub) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <span
                      className="w-3 h-3 rounded-full ring-2 ring-white/20 shadow-xs shrink-0"
        ...
        {/* Drawer Footer */}
        <div className="p-4 border-t border-slate-800/90 bg-slate-900/60 flex flex-col gap-3">
          <form action={signOut}>
            <button
              type="submit"
              className="w-full py-2.5 px-3 text-slate-400 hover:text-rose-400 hover:bg-rose-950/40 border border-transparent hover:border-rose-800/40 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all"
            >
              <LogOut size={16} />
              <span>Sign Out</span>
            </button>
