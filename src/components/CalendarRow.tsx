          ...
          return (
            ...
            >
              <span className={`text-[10px] font-bold tracking-wider mb-1 transition-colors ${
                isSelected ? 'text-blue-500' : 'text-stone-400 group-hover:text-stone-600'
              }`}>
                {formatDay(day)}
              </span>
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold transition-all ${
                  isSelected
                    ? 'bg-gradient-to-tr from-blue-600 to-blue-500 text-white shadow-md shadow-blue-900/20 ring-2 ring-blue-400/30'
                    : isToday
                    ? 'bg-amber-100/80 text-blue-500 font-extrabold ring-1 ring-amber-300'
              ...
              <div className="h-4 mt-1 flex items-center justify-center">
                {count > 0 ? (
                  <span className={`text-[10px] font-bold px-1.5 py-0.2 rounded-full ${
                    isSelected ? 'bg-blue-100 text-blue-500' : 'bg-emerald-100 text-emerald-700'
                  }`}>
                    {count}
                  </span>
                ) : (
